...
# --- Refactored & Optimized Functions ---
...
def add_to_cart(product_id, quantity):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        if product:
            ...
            # تحويل السعر إلى العملة الأساسية للتخزين (USD)
            base_usd_price = convert_price(float(price_val), prod_currency, "USD ($)")
...
def generate_pdf_invoice():
    ...
    t = Table(data, colWidths=[130, 70, 30, 80])
...
def add_product_page():
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor.execute("INSERT INTO products (name, price, currency, image) VALUES (?, ?, ?, ?)",
                       (data['name'], str(data['price']), data['currency'], image_str))
        conn.commit()
...
def list_products_page():
    ...
    if not products:
        ...
        for prod in products:
            ...
            img_src = get_image_source(img_path)
            disp_price = convert_price(float(base_price), item_currency, selected_currency)
...
def edit_product_page(product_id):
    ...
    with sqlite3.connect(DB_NAME) as conn:
        ...
        cursor.execute("UPDATE products SET name = ?, price = ?, currency = ?, image = ? WHERE id = ?",
                       (data['name'], str(data['price']), data['currency'], image_str, product_id))
        conn.commit()
